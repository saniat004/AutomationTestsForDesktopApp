...
namespace DesktopApp.UITests
{
    [TestClass]
    public class DesktopAppTests
    {
            [TestMethod]
            public void CheckDetailAndBack()
            {
                var driver = StartApp();
                // tap on second item
        ...
        //method for starting the app
            public WindowsDriver<WindowsElement> StartApp()
            {
                ...
                capabilities.AddAdditionalCapability(MobileCapabilityType.App, @"D:\Desktop UI tests\AppforWpf\bin\Debug\CarvedRock.Wpf.exe");
